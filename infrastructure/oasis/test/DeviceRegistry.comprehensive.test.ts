...
describe("DeviceRegistry - Comprehensive Tests", function () {
  ...
  describe("Device Registration", function () {
    it("Should register multiple devices", async function () {
      ...
      for (const device of devices) {
        const publicKeyHash = ethers.keccak256(ethers.encodeBytes32String(device.key));
        const deviceFingerprint = ethers.encodeBytes32String(device.fingerprint);

        await expect(
          deviceRegistry.connect(user).registerDevice(device.name, publicKeyHash, deviceFingerprint)
        ).to.emit(deviceRegistry, "DeviceRegistered");
      }

      const userDevices = await deviceRegistry.getUserDevices(await user.getAddress());
    ...
    it("Should prevent registering too many devices", async function () {
      ...
      for (let i = 0; i < 10; i++) {
        const publicKeyHash = ethers.keccak256(ethers.encodeBytes32String(`key${i}`));
      ...
      const publicKeyHash = ethers.keccak256(ethers.encodeBytes32String("key11"));
      const deviceFingerprint = ethers.encodeBytes32String("fp11");

      await expect(
        deviceRegistry.connect(user).registerDevice("Device 11", publicKeyHash, deviceFingerprint)
    ...
    it("Should require valid device name", async function () {
      ...
      await expect(
        deviceRegistry.connect(user).registerDevice("", publicKeyHash, deviceFingerprint)
    ...
    it("Should require valid public key hash", async function () {
      ...
      await expect(
        deviceRegistry.connect(user).registerDevice("Device", ethers.ZeroHash, deviceFingerprint)
    ...
    it("Should get device information", async function () {
      ...
      const registerTx = await deviceRegistry.connect(user).registerDevice(
        deviceName,
        publicKeyHash,
        deviceFingerprint
      );
  ...
  describe("Device Authentication", function () {
    ...
    beforeEach(async function () {
      ...
      const publicKeyHash = ethers.keccak256(ethers.encodeBytes32String("auth-key"));
    ...
    it("Should generate authentication challenges", async function () {
      const challenge = await deviceRegistry.connect(user).generateAuthChallenge(deviceId);
      expect(challenge).to.not.equal(ethers.ZeroHash);

      // Generate another challenge - should be different
      const challenge2 = await deviceRegistry.connect(user).generateAuthChallenge(deviceId);
      expect(challenge2).to.not.equal(challenge);
    });
    ...
    it("Should authenticate device with valid signature", async function () {
      ...
      await expect(
        deviceRegistry.connect(user).authenticateDevice(deviceId, challenge, signature)
    ...
    it("Should record authentication history", async function () {
      ...
      await deviceRegistry.connect(user).authenticateDevice(deviceId, challenge, signature);

      const history = await deviceRegistry.connect(user).getDeviceAuthHistory(deviceId, 10);
    ...
    it("Should get device status", async function () {
      const status = await deviceRegistry.connect(user).getDeviceStatus(deviceId);
  ...
  describe("Device Management", function () {
    ...
    beforeEach(async function () {
      ...
      const publicKeyHash = ethers.keccak256(ethers.encodeBytes32String("mgmt-key"));
    ...
    it("Should suspend device", async function () {
      ...
      const status = await deviceRegistry.connect(user).getDeviceStatus(deviceId);
      expect(status).to.equal(2); // Suspended status
    ...
    it("Should revoke device", async function () {
      await expect(
        deviceRegistry.connect(user).revokeDevice(deviceId)
      ).to.emit(deviceRegistry, "DeviceRevoked");

      const status = await deviceRegistry.connect(user).getDeviceStatus(deviceId);
      expect(status).to.equal(1); // Revoked status
  ...
  describe("Access Control", function () {
    ...
    beforeEach(async function () {
      ...
      const publicKeyHash = ethers.keccak256(ethers.encodeBytes32String("ac-key"));
    ...
    it("Should allow owner or device owner to get device status", async function () {
      ...
      const status1 = await deviceRegistry.connect(user).getDeviceStatus(deviceId);
      expect(status1).to.equal(0);

      // Contract owner can access
      const status2 = await deviceRegistry.connect(owner).getDeviceStatus(deviceId);
  ...
  describe("Admin Functions", function () {
    it("Should allow owner to set max devices per user", async function () {
      await deviceRegistry.connect(owner).setMaxDevicesPerUser(ethers.ZeroAddress, 5);
      ...
      const publicKeyHash = ethers.keccak256(ethers.encodeBytes32String("key6"));
      const deviceFingerprint = ethers.encodeBytes32String("fp6");

      await expect(
        deviceRegistry.connect(user).registerDevice("Device 6", publicKeyHash, deviceFingerprint)
      ).to.be.revertedWith("Too many devices");
    });
    ...
    it("Should allow owner to emergency revoke device", async function () {
      ...
      const publicKeyHash = ethers.keccak256(ethers.encodeBytes32String("emergency-key"));
      ...
      await expect(
        deviceRegistry.connect(owner).emergencyRevokeDevice(deviceId, "Security breach")
      ).to.emit(deviceRegistry, "DeviceRevoked");

      const status = await deviceRegistry.connect(owner).getDeviceStatus(deviceId);
      expect(status).to.equal(1); // Revoked status
    });
    ...
    it("Should allow owner to pause/unpause", async function () {
      ...
      await expect(
        deviceRegistry.connect(user).registerDevice("Device", publicKeyHash, deviceFingerprint)
  ...
  describe("Event Emissions", function () {
    it("Should emit generic vault events", async function () {
      await deviceRegistry.emitVaultEvent(await user.getAddress(), 1, ethers.toUtf8Bytes("test data"));
    });
