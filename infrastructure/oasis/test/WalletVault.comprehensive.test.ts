...
describe("WalletVault - Comprehensive Tests", function () {
  ...
  describe("Wallet Management", function () {
    it("Should prevent creating too many wallets", async function () {
      ...
      for (let i = 0; i < 10; i++) {
        await walletVault.connect(user).importSeedPhrase(encryptedSeed, `Wallet ${i}`);
      }
    ...
    it("Should update wallet metadata", async function () {
      ...
      const importTx = await walletVault.connect(user).importSeedPhrase(encryptedSeed, walletName);
      ...
      await walletVault.connect(user).updateWalletMetadata(walletId, "Updated Wallet", false);
  ...
  describe("Multi-Chain Operations", function () {
    ...
    it("Should derive keys for all supported chains", async function () {
      const chainTypes = [1, 2, 3, 4, 5]; // All supported chains

      await walletVault.connect(user).deriveKeysFromSeed(walletId, chainTypes);

      const walletInfo = await walletVault.connect(user).getWalletInfo(walletId);
      expect(walletInfo.chainTypes).to.have.length(5);
    });
    ...
    it("Should fetch balances for derived addresses", async function () {
      ...
      const balances = await walletVault.connect(user).fetchWalletBalances(walletId);
    ...
    it("Should get derived address for specific chain", async function () {
      ...
      const address = await walletVault.connect(user).getDerivedAddress(walletId, 1);
    ...
    it("Should sign transactions", async function () {
      ...
      await expect(
        walletVault.connect(user).signTransaction(walletId, 1, txHash, txData)
      ).to.emit(walletVault, "TransactionSigned");
    });
    ...
    it("Should reject unsupported chain types", async function () {
      ...
      await expect(
        walletVault.connect(user).deriveKeysFromSeed(walletId, invalidChainTypes)
  ...
  describe("Multi-Chain RPC Functions", function () {
    it("Should get multi-chain balances for address", async function () {
      ...
      const balances = await walletVault.getMultiChainBalances(testAddress, chainTypes);
      expect(balances).to.have.length(2);
    });

    it("Should execute RPC calls", async function () {
      const result = await walletVault.executeChainRPC(1, "eth_getBalance", ethers.toUtf8Bytes("params"));
    ...
    it("Should batch get balances", async function () {
      ...
      const balances = await walletVault.batchGetBalances(addresses, chainTypes);
      expect(balances).to.have.length(2);
  ...
  describe("Access Control & Admin Functions", function () {
    ...
    it("Should get user wallets", async function () {
      ...
      await walletVault.connect(user).importSeedPhrase(encryptedSeed, "Wallet 1");
      await walletVault.connect(user).importSeedPhrase(encryptedSeed, "Wallet 2");

      const userWallets = await walletVault.getUserWallets(await user.getAddress());
  ...
  describe("Event Emissions", function () {
    it("Should emit generic vault events", async function () {
      await walletVault.emitVaultEvent(await user.getAddress(), 1, ethers.toUtf8Bytes("test data"));
    });
