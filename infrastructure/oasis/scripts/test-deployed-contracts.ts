...
async function main() {
  console.log("🧪 Testing deployed Grand Warden contracts on Sapphire testnet...\n");

  const [deployer, user] = await ethers.getSigners();
  console.log(`Testing with deployer: ${await deployer.getAddress()}`);
  console.log(`Testing with user: ${user ? await user.getAddress() : 'Using deployer as user'}\n`);
  ...
  const grandWardenVault = await ethers.getContractAt("GrandWardenVault", addresses.GrandWardenVault);
  const walletVault = await ethers.getContractAt("WalletVault", addresses.WalletVault);
  const deviceRegistry = await ethers.getContractAt("DeviceRegistry", addresses.DeviceRegistry);
  const recoveryManager = await ethers.getContractAt("RecoveryManager", addresses.RecoveryManager);
  const multiChainRPC = await ethers.getContractAt("MultiChainRPC", addresses.MultiChainRPC);
  const atomicVaultManager = await ethers.getContractAt("AtomicVaultManager", addresses.AtomicVaultManager);
  ...
  try {
    ...
    const createTx = await grandWardenVault.connect(testUser).createVault(vaultData);
  ...
  if (vaultId) {
    await runTest("Add credential to vault", async () => {
      ...
      const tx = await grandWardenVault.connect(testUser).addCredential(
        vaultId, domain, username, encryptedPassword
      );
      const receipt = await tx.wait();
      
      const event = receipt?.logs.find(log => 
        grandWardenVault.interface.parseLog(log as any)?.name === "CredentialAdded"
      );
    ...
    await runTest("Update vault blob", async () => {
      ...
      const tx = await grandWardenVault.connect(testUser).updateVaultBlob(vaultId, newBlob);
      await tx.wait();
    });
  }
  ...
  await runTest("Import seed phrase", async () => {
    ...
    const tx = await walletVault.connect(testUser).importSeedPhrase(encryptedSeed, walletName);
    const receipt = await tx.wait();
  ...
  try {
    ...
    const importTx = await walletVault.connect(testUser).importSeedPhrase(encryptedSeed, "Test Wallet 2");
  ...
  if (walletId) {
    await runTest("Derive keys for multiple chains", async () => {
      ...
      const tx = await walletVault.connect(testUser).deriveKeysFromSeed(walletId, chainTypes);
      await tx.wait();
      
      const walletInfo = await walletVault.connect(testUser).getWalletInfo(walletId);
      if (walletInfo.chainTypes.length !== 2) {
        throw new Error(`Expected 2 chains, got ${walletInfo.chainTypes.length}`);
      }
    });

    await runTest("Get derived address", async () => {
      const address = await walletVault.connect(testUser).getDerivedAddress(walletId, 1);
    ...
    await runTest("Fetch wallet balances", async () => {
      const balances = await walletVault.connect(testUser).fetchWalletBalances(walletId);
  ...
  await runTest("Register device", async () => {
    ...
    const publicKeyHash = ethers.keccak256(ethers.encodeBytes32String("test-key"));
    const fingerprint = ethers.encodeBytes32String("test-fingerprint");
    
    const tx = await deviceRegistry.connect(testUser).registerDevice(
      deviceName, publicKeyHash, fingerprint
    );
    const receipt = await tx.wait();
  ...
  try {
    ...
    const publicKeyHash = ethers.keccak256(ethers.encodeBytes32String("test-key-2"));
    const fingerprint = ethers.encodeBytes32String("test-fingerprint-2");
    
    const registerTx = await deviceRegistry.connect(testUser).registerDevice(
      deviceName, publicKeyHash, fingerprint
    );
  ...
  if (deviceId) {
    await runTest("Generate authentication challenge", async () => {
      const challenge = await deviceRegistry.connect(testUser).generateAuthChallenge(deviceId);
    ...
    await runTest("Authenticate device", async () => {
      ...
      const tx = await deviceRegistry.connect(testUser).authenticateDevice(deviceId, challenge, signature);
      const receipt = await tx.wait();
      
      const event = receipt?.logs.find(log => 
        deviceRegistry.interface.parseLog(log as any)?.name === "DeviceAuthenticated"
      );
  ...
  await runTest("Add guardian", async () => {
    ...
    const contactHash = ethers.keccak256(ethers.toUtf8Bytes("test@guardian.com"));
    
    const tx = await recoveryManager.connect(testUser).addGuardian(
      guardianAddress, guardianName, contactHash
    );
    const receipt = await tx.wait();
  ...
  await runTest("Get user guardians", async () => {
    const guardians = await recoveryManager.getUserGuardians(await testUser.getAddress());
  ...
  await runTest("Create recovery shares", async () => {
    ...
    const tx = await recoveryManager.connect(testUser).createRecoveryShares(threshold, shareData);
    const receipt = await tx.wait();
    
    const event = receipt?.logs.find(log => 
      recoveryManager.interface.parseLog(log as any)?.name === "RecoveryShareCreated"
    );
    if (!event) throw new Error("RecoveryShareCreated event not found");
  });

  console.log("\n🌐 Testing Multi-Chain RPC (MultiChainRPC):");

  await runTest("Get chain configuration", async () => {
    const config = await multiChainRPC.getChainConfig(1);
    if (config.name !== "Ethereum") {
      throw new Error(`Expected Ethereum, got ${config.name}`);
    }
  });

  await runTest("Get all supported chains", async () => {
    const chains = await multiChainRPC.getAllChains();
    if (chains.length === 0) {
      throw new Error("No chains configured");
    }
  });

  await runTest("Get multi-chain balances", async () => {
    const testAddress = await testUser.getAddress();
    const chainTypes = [1, 2];
    
    const balances = await multiChainRPC.getMultiChainBalances(testAddress, chainTypes);
    if (balances.length !== 2) {
      throw new Error(`Expected 2 balances, got ${balances.length}`);
    }
  });

  await runTest("Execute RPC call", async () => {
    const result = await multiChainRPC.executeChainRPC(
      1, 
      "eth_getBalance", 
      ethers.AbiCoder.defaultAbiCoder().encode(["address"], [await testUser.getAddress()])
    );
    if (result.length === 0) {
  ...
  await runTest("Execute atomic update", async () => {
    const testVaultId = ethers.keccak256(ethers.toUtf8Bytes("test-atomic-vault"));
    const testData = ethers.toUtf8Bytes("test atomic data");
    
    const tx = await atomicVaultManager.connect(testUser).executeAtomicUpdate(testVaultId, testData);
    const receipt = await tx.wait();
    
    const event = receipt?.logs.find(log => 
      atomicVaultManager.interface.parseLog(log as any)?.name === "AtomicUpdateStarted"
    );
  ...
  if (successRate >= 90) {
    ...
    console.log("✅ Ready to proceed with frontend integration and Phase 2 (The Graph)");
  ...
  Object.entries(addresses).forEach(([name, address]) => {
    console.log(`   ${name}: https://explorer.sapphire.oasis.io/address/${address}`);
  });
