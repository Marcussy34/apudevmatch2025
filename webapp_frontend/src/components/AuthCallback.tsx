...
import { autoWalExchange } from "../services/backend-integration";

// Global flag to prevent multiple executions across component instances
...
const AuthCallback: React.FC<AuthCallbackProps> = ({ onLogin }) => {
  ...
  useEffect(() => {
    ...
    console.log("🔄 Global isProcessing:", isProcessing);

    // Prevent multiple executions (both local and global)
    if (hasProcessedRef.current || isProcessing) {
      console.log("⏭️ Skipping - already processed or processing");
      return;
    }
    ...
    const handleCallback = async () => {
      try {
        ...
        console.log("✅ Global isProcessing set to true");
        console.log("✅ hasProcessed flag set to true");
        setCurrentStep(0);
        ...
        if (!idToken) {
          setStatus("error");
          setErrorMessage(
            "No ID token received from Google. Please check your OAuth configuration."
          );
          return;
        }
        ...
        // Automatically exchange SUI for WAL tokens after successful zkLogin
        try {
          console.log("🪙 Starting automatic WAL exchange after zkLogin...");
          const exchangeResult = await autoWalExchange({
            senderAddress: userProfile.suiAddress,
            amountMist: 500_000, // 0.5 SUI
          });
          console.log("✅ WAL exchange completed:", exchangeResult.digest);
        } catch (error) {
          console.error(
            "❌ WAL exchange failed, but zkLogin was successful:",
            error
          );
          console.error(
            "❌ WAL exchange error details:",
            error instanceof Error ? error.message : error
          );
          console.error(
            "❌ WAL exchange error stack:",
            error instanceof Error ? error.stack : "No stack trace"
          );
          // Don't fail the entire login flow if WAL exchange fails
        }
        ...
        isProcessing = false;
        console.log("🔄 Global isProcessing reset to false");
      }
    };
